    ...
    public class MethodWriter
    {
        ...
        private void WriteInternalTransitionMethodsForState(WriteContext<StateMachine> context, Transition[] uniqueTransitions, string state)
        {
            ...
            foreach (var transition in internalTransitions)
            {
                ...
                context.Writer.WriteLine($"private {(transition.IsAsync ? "Task" : "void")} {transitionMethodName}({typedNamedParameters1}{MicroMachineWriter.StateMachineType}.Transition transition) => {transitionMethodName}({namedParameters});");
                context.Writer.WriteLine();
            }
        }
