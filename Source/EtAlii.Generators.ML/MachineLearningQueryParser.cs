namespace EtAlii.Generators.ML
{
    ...
    {
        private readonly ILogger _log = Log.ForContext<MachineLearningQueryParser>();
        ...
        public bool TryParse(AdditionalText file, out object stateMachine, out Diagnostic[] diagnostics)
        {
            ...
            try
            {
                ...
                _log
                    ...
                    .Information("Parsing ML query {File}", file.Path);
                ...
                if (success)
                {
                    _log.Information("Successfully parsed ML query file");
                }
                else
                {
                    _log.Error("Unable to parse ML query file");
                    stateMachine = null;
                }
            }
            catch (Exception e)
            {
                _log.Fatal(e, "Failure parsing ML query file");
